...
It is taken from aiida-lsmo:
https://github.com/lsmo-epfl/aiida-lsmo/tree/develop/aiida_lsmo/calcfunctions

to adopt it for multicomponent isotherm simulations as well as VLCC.

In the case of former one, the ff_data.yaml needs some changes in naming as
RASPA way of reading force field parameters does now handle the N_n2 and M_n2 (N2 for example)
properly which leads to errornous simulations. These need to be back to RASPA convention, ie. 
N_n2 and N_com.

In the case of VLCC, there is a small change in this file where we do not read and write atoms
from UFF for framework atoms as there is no framework. 
"""
import tempfile
import shutil
import os
...
def render_ff_mixing_def(ff_data, params):
    ...
    if params['ff_framework'] is not None:
